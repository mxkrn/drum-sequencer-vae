...
from dsvae.data.converter.base import BaseNoteSequenceConverter, ConverterTensors
...
class GrooveConverter(BaseNoteSequenceConverter):
    ...
    def __init__(
        ...
        max_note_dropout_probability=0,
        onsets_only=False,
    ):
    ...
    def to_tensors(self, note_sequence):
        def _get_steps_hash(note_sequence):
            ...
            for note in note_sequence.notes:
                ...
                if note.pitch in self.pitch_class_map.keys():
        ...
        # Now concatenate all 3 vectors into 1, eg (16, 27)... or onsets_only.
        if self._onsets_only:
            seqs = hit_vectors
            input_seqs = hit_vectors
            # seqs = np.concatenate([hit_vectors], axis=1)
            # input_seqs = np.concatenate([in_hits], axis=1)
        else:
            seqs = np.concatenate(
